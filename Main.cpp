...
//	TFontStyles
	...
	ACanvas->TextRect(ARect, ARect.Left + 3, ARect.Top + 3, "Файл   ");
...
void __fastcall TFMain::ToolButton_StartClick(TObject *Sender)
{
	if (GBISystem->IsRunning() == false) {
		...
		Console("Система", "Запуск инклинометра");
	...
	else
	{
		...
		Console("Система", "Останов инклинометра");
		MMFile->Enabled = true;
	}
...
{
	Console("Приложение","Старт системы ...");
	...
	//Применяем загруженные параметры системы
	...
	Console("Приложение","Инициалиация системы прошла успешно");
	b_system_start = false;

	if (GBISystem->SysConfMgr.auto_backup_flag_completed) {

		Console("Приложение","Автоматическое резервное копирование БД выполнено");
	}
...
void TFMain::ApplicationInit()
{
	...
	AnsiString sver("");
	sver.printf("Версия: %d.%d.%d", BUILD, VERSION, SUBVERSION);
...
void __fastcall TFMain::Timer_system_runTimer(TObject *Sender)
{
		...
		{
			if (comerr == true) Console("Система", "Не удалось открыть СОМ-порт");
		...
		else
		{
			if (comerr == false) {
				 ...
				 Console("Система", "Есть ответ СОМ-порта");
			}
		  ...
		  TRect &Rect, TGridDrawState State)
{
	...
	if (ARow == 0) {
			...
			if (ACol == 0) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Номер");
			if (ACol == 1) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Уровень");
			if (ACol == 2) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Погр.трб.");
			...
			if (ACol == 7) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Xрез");
			if (ACol == 8) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Yрез");
			...
			if (ACol == 11) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Рез.см.");
			if (ACol == 12) StringGrid_meas->Canvas->TextOut(Rect.Left, Rect.Top, "Рез.уг.");
...
void __fastcall TFMain::Button_recordClick(TObject *Sender)
{
	...
	if (selected_meas == NULL) {
		...
		utils_ShowMessage("Выберите измрение в дереве системы!");
		return;
	}
	...
	if (selected_meas->finalized) {
		...
		if (Application->MessageBoxA(L"Данное измерение уже завершено! Хотите продолжить сбор данных?",L"ВНИМАНИЕ!",1)!=IDOK) {
			return;
		}
	  ...
	  {
		  if (Application->MessageBoxA(L"Зонд не был перемещен. Хотите записать данные повторно?",L"ВНИМАНИЕ!",1) != IDOK)
		  {
			return;
		  }
	  }
...
void __fastcall TFMain::StringGrid_measDblClick(TObject *Sender)
{
	...
	if (selected_meas == NULL) {
		...
		utils_ShowMessage("Выберите измерение в дереве системы!");
		return;
	}
...
void __fastcall TFMain::N_place_deleteClick(TObject *Sender)
{
	...
	s.printf("Вы действительно хотите удалить место [%s] ?",current_place->GetName().c_str());

    WideChar mes[1024];
	StringToWideChar(s.c_str(),mes,1024);

	if (Application->MessageBoxA(mes,L"Внимание!",1)!=IDOK) return;
...
void __fastcall TFMain::MMAboutClick(TObject *Sender)
{
	...
	AnsiString s;
	s.printf("Версия: %d.%d.%d", BUILD, VERSION, SUBVERSION);
...
void __fastcall TFMain::N_drill_adjustClick(TObject *Sender)
{
	...
	{
		if(Application->MessageBoxA(L"Параметры скважины изменились. Выполнить перерасчет всех измерений?",L"ВНИМАНИЕ!",1) == IDOK )
		{
...
void __fastcall TFMain::N_drill_deleteClick(TObject *Sender)
{
	...
	s.printf("Вы действительно хотите удалить скважину [%s] ?",d->name.c_str());
...
void __fastcall TFMain::N_meas_addClick(TObject *Sender)
{
	...
	d->AddMeas(StringGrid_meas, "Измерение");
...
void __fastcall TFMain::N_meas_deleteClick(TObject *Sender)
{
	...
	s.printf("Вы действительно хотите удалить измерение [%s] ?",m->name.c_str());
...
void TFMain::ViewSelectedMeas(void)
{
	...
	else
	{
		...
		if (d->drill_orient == DRILL_ORIENT_VERTICAL)
		{
			s.printf("%s [вертикальная][%.1f]",d->name.c_str(), d->drill_orient);
		}
		else
		{
			s.printf("%s [горизонтальная]",d->name.c_str());
		}
...
void __fastcall TFMain::Button_finishClick(TObject *Sender)
{
	...
	if (selected_meas->finalized) {
		...
		if (Application->MessageBoxA(L"Данное измерение уже завершено, время завершения будет изменено. Хотите продолжить?",L"ВНИМАНИЕ!",1) != IDOK)  {
...
void __fastcall TFMain::ToolButton11Click(TObject *Sender)
{
	...
	Console("Приложение","Ручное резервное копирование БД выполнено");
}
//---------------------------------------------------------------------------
...
void __fastcall TFMain::ToolButton12Click(TObject *Sender)
{
		...
		{
			utils_ShowMessage("Выберите измерение или скважину в дереве системы!");
		}
}
//---------------------------------------------------------------------------
...
//	  else
	  {
		  AnsiString s("");
		  s.printf("Экспорт в Excel ... %.1f%",excel_export_rate);
